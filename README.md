...
- 공식 공지 데이터는 외부 수집 파이프라인(gms-tracker)을 통해 Supabase에 적재하고 앱은 `events_v2`, `maintenance_v2`를 조회하는 구조로 설계했습니다
...
- `pnpm lint`: 정적 분석
- `pnpm test`: 날짜 변환 및 정렬 유틸 테스트
- `pnpm build`: 프로덕션 빌드 확인

참고로 `pnpm lint`, `pnpm test`는 문서와 유틸 로직 검증용으로 바로 실행할 수 있고 `pnpm dev`, `pnpm build`로 실제 화면을 확인하려면 Supabase 환경변수와 데이터가 준비되어 있어야 합니다
